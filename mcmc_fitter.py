...
def gaussian_process_update(
    theta, x_train, y_train, x_test, kernel_func, mean_func, noise_variance
):
    ...
    K = kernel_func(x_train, x_train, sigma_f=1, sigma_l=2) + noise_variance * np.eye(
        n
    )  # Add noise to the diagonal
    K_s = kernel_func(
        x_train, x_test, sigma_f=1, sigma_l=2
    )  # Cross-covariance between training and test
    K_ss = kernel_func(
        x_test, x_test, sigma_f=1, sigma_l=2
...
def mcmc_fit(time, magnifications, bounds, initial, numiter, noise):
    ...
    def log_prior(theta):
        ...
        if (
            t_E_bounds[0] < t_E < t_E_bounds[1]
            and t_0_bounds[0] < t_0 < t_0_bounds[1]
            and u_0_bounds[0] < u_0 < u_0_bounds[1]
        ):
            return 0.0
        else:
            return -np.inf
    ...
    def log_probability(theta, time, magnifications):
        ...
        mu, cov = gaussian_process_update(
            theta,
            time.reshape(-1, 1),
            magnifications.reshape(-1, 1),
            time.reshape(-1, 1),
            GP.matern52_kernel,
            mean_function_theta,
            noise,
    ...
    sampler = emcee.EnsembleSampler(
        nwalkers, ndim, log_probability, args=(time, magnifications)
    )
...
def mcmc_results(sampler, burnin=0):
    ...
    print(
        f"t_E = {params[1,0]:.2f} + {params[2,0]-params[1,0]:.2f} - {params[1,0]-params[0,0]:.2f}"
    )
    print(
        f"u_0 = {params[1,1]:.2f} + {params[2,1]-params[1,1]:.2f} - {params[1,1]-params[0,1]:.2f}"
    )
    print(
        f"t_E = {params[1,2]:.2f} + {params[2,2]-params[1,2]:.2f} - {params[1,2]-params[0,2]:.2f}"
    )
